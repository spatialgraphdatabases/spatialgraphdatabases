...
<svg viewBox="0 0 760 478" role="img" aria-labelledby="planTitle planDesc" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;font-family:var(--font-sans,sans-serif)">
  ...
  <rect class="viz-backdrop" x="0" y="0" width="760" height="478" fill="var(--viz-bg,#ffffff)"/>
...
<svg viewBox="0 0 780 316" role="img" aria-labelledby="pbTitle pbDesc" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;font-family:var(--font-sans,sans-serif)">
  <title id="pbTitle">The WITH boundary is what forces the spatial predicate to resolve before traversal</title>
  <desc id="pbDesc">Two operator trees for the same question. Without the WITH boundary the planner is free to interleave, so a label scan feeds the SERVES expansion and the distance filter runs above it, on rows that were produced by walking relationships from every hub in the label. With the WITH boundary the spatial predicate must resolve to a complete row set first, so the base operator is a point index seek that materialises only the hubs inside the radius, and the expansion runs from that reduced set.</desc>
  <defs>
    <marker id="pb-a" viewBox="0 0 10 10" refX="8.5" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
      <path d="M0 0 L10 5 L0 10 z" fill="currentColor"/>
    </marker>
  </defs>
  <rect class="viz-backdrop" x="0" y="0" width="780" height="316" fill="var(--viz-bg,#ffffff)"/>
  <text x="196" y="24" text-anchor="middle" font-size="13" font-weight="700" fill="var(--viz-poor,#a8320f)">no WITH boundary</text>
  <text x="584" y="24" text-anchor="middle" font-size="13" font-weight="700" fill="var(--viz-good,#0a656d)">WITH hub before the expansion</text>
  ...
  <g font-size="11" text-anchor="middle" font-weight="700">
    <rect x="52" y="240" width="288" height="40" rx="8" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-poor,#a8320f)" stroke-width="2"/>
    <text x="196" y="258" fill="var(--viz-poor,#a8320f)">NodeByLabelScan</text>
    <text x="196" y="273" font-size="9.5" font-weight="400" fill="var(--viz-ink-mute,#565f6d)">180,000 hubs</text>
    <rect x="52" y="176" width="288" height="40" rx="8" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-stroke,#9ca3af)" stroke-width="1.4"/>
    <text x="196" y="194" fill="currentColor">Expand(All) :SERVES</text>
    <text x="196" y="209" font-size="9.5" font-weight="400" fill="var(--viz-ink-mute,#565f6d)">1,240,000 rows</text>
    <rect x="52" y="112" width="288" height="40" rx="8" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-poor,#a8320f)" stroke-width="1.6"/>
    <text x="196" y="130" fill="currentColor">Filter point.distance()</text>
    <text x="196" y="145" font-size="9.5" font-weight="400" fill="var(--viz-ink-mute,#565f6d)">1,240,000 distance computations</text>
    <rect x="52" y="62" width="288" height="30" rx="8" fill="var(--viz-panel-2,#ece9df)" stroke="var(--viz-stroke-soft,#cdc6b3)" stroke-width="1.2"/>
    <text x="196" y="82" font-size="10.5" fill="currentColor">312 rows returned</text>
  </g>
  <g stroke="currentColor" stroke-width="1.6" marker-end="url(#pb-a)">
    <line x1="196" y1="240" x2="196" y2="220"/><line x1="196" y1="176" x2="196" y2="156"/><line x1="196" y1="112" x2="196" y2="96"/>
  </g>
  <g font-size="11" text-anchor="middle" font-weight="700">
    <rect x="440" y="240" width="288" height="40" rx="8" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-good,#0a656d)" stroke-width="2"/>
    <text x="584" y="258" fill="var(--viz-good,#0a656d)">PointIndexSeekByRange</text>
    <text x="584" y="273" font-size="9.5" font-weight="400" fill="var(--viz-ink-mute,#565f6d)">410 hubs in the bounding region</text>
    <rect x="440" y="176" width="288" height="40" rx="8" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-good,#0a656d)" stroke-width="1.6"/>
    <text x="584" y="194" fill="currentColor">Filter point.distance()</text>
    <text x="584" y="209" font-size="9.5" font-weight="400" fill="var(--viz-ink-mute,#565f6d)">410 distance computations</text>
    <rect x="440" y="112" width="288" height="40" rx="8" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-stroke,#9ca3af)" stroke-width="1.4"/>
    <text x="584" y="130" fill="currentColor">Expand(All) :SERVES</text>
    <text x="584" y="145" font-size="9.5" font-weight="400" fill="var(--viz-ink-mute,#565f6d)">2,800 rows</text>
    <rect x="440" y="62" width="288" height="30" rx="8" fill="var(--viz-panel-2,#ece9df)" stroke="var(--viz-stroke-soft,#cdc6b3)" stroke-width="1.2"/>
    <text x="584" y="82" font-size="10.5" fill="currentColor">312 rows returned</text>
  </g>
  <g stroke="currentColor" stroke-width="1.6" marker-end="url(#pb-a)">
    <line x1="584" y1="240" x2="584" y2="220"/><line x1="584" y1="176" x2="584" y2="156"/><line x1="584" y1="112" x2="584" y2="96"/>
  </g>
  <text x="24" y="306" font-size="10.5" fill="var(--viz-ink-mute,#565f6d)">Same answer, same 312 rows. The difference is that one tree computes 1.24 million great-circle distances to discard almost all of them.</text>
</svg>
...
It is worth being precise about what the barrier does and does not guarantee, because `WITH` is often described as "forcing" an order and that is a little stronger than the truth. `WITH` establishes a scope boundary: everything before it must produce a complete row set before anything after it runs. That is enough to stop the planner from interleaving the expansion into the spatial resolution, which is the reordering this query needs prevented. It is not a promise that the resolution itself will use the index — if the predicate above the barrier is not seekable, the barrier simply guarantees a complete label scan before the expansion instead of an interleaved one. The barrier and the seekable predicate are two requirements, and satisfying one without the other buys nothing.

The same caution applies to reading the improvement. A plan that moves from scan-then-expand to seek-then-expand will show a large drop in `DbHits` and usually in wall time, but the number to check afterwards is the ratio of rows entering the final filter to rows leaving it. A seek that hands 400 candidates to a distance filter which keeps 312 is working as intended; one that hands over 40,000 to keep the same 312 is a seek in name only — the bounding region is far larger than the radius, typically because the box was computed with a fixed degree offset rather than one corrected for latitude. That correction matters more the further from the equator the data sits, and it is invisible in the plan, which reports a healthy index seek in both cases.
