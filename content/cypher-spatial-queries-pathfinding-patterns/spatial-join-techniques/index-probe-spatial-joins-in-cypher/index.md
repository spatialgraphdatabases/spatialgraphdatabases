...
<svg viewBox="0 0 780 400" role="img" aria-labelledby="ipTitle ipDesc" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;font-family:var(--font-sans,sans-serif)">
  ...
  <rect class="viz-backdrop" x="0" y="0" width="780" height="400" fill="var(--viz-bg,#ffffff)"/>
...
<svg viewBox="0 0 780 322" role="img" aria-labelledby="pniTitle pniDesc" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;font-family:var(--font-sans,sans-serif)">
  <title id="pniTitle">The same probe query is fast or catastrophic depending only on the index state</title>
  <desc id="pniDesc">Three plan shapes for one join, with the work each does over 2,000 driving rows and 400,000 road nodes. A comma MATCH plans as CartesianProduct and evaluates 800 million pairs. Splitting into an Apply with a box predicate is correct but, with no online point index, the inner side is a NodeByLabelScan re-run per outer row — the same 800 million node reads, now with per-row setup on top. The same Apply over an ONLINE point index plans as PointIndexSeekByRange and touches roughly 60,000 nodes. The middle shape is the trap: the query text already looks fixed.</desc>
  <rect class="viz-backdrop" x="0" y="0" width="780" height="322" fill="var(--viz-bg,#ffffff)"/>
  <text x="24" y="24" font-size="13.5" font-weight="700" fill="currentColor">Splitting the MATCH is half the fix — the index state is the other half</text>
  <text x="24" y="42" font-size="10.5" fill="var(--viz-ink-mute,#565f6d)">2,000 driving rows × 400,000 :RoadNode</text>
  <rect x="24" y="58" width="732" height="72" rx="9" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-poor,#a8320f)" stroke-width="1.6"/>
  <text x="42" y="82" font-size="11.5" font-weight="700" fill="currentColor">One MATCH, comma-separated</text>
  <text x="42" y="100" font-size="10" fill="var(--viz-ink-mute,#565f6d)">(e:PickupEvent), (n:RoadNode) — no shared variable</text>
  <rect x="300" y="70" width="150" height="22" rx="11" fill="var(--viz-poor,#a8320f)"/>
  <text x="375" y="86" text-anchor="middle" font-size="10" font-weight="700" fill="var(--viz-on-pill,#ffffff)">CartesianProduct</text>
  <text x="42" y="120" font-size="10" fill="var(--viz-poor,#a8320f)" font-weight="600">800,000,000 pairs evaluated</text>
  <rect x="470" y="70" width="270" height="46" rx="6" fill="var(--viz-panel-2,#ece9df)" stroke="var(--viz-stroke-soft,#cdc6b3)" stroke-width="1"/>
  <rect x="478" y="86" width="254" height="14" rx="7" fill="var(--viz-poor,#a8320f)"/>
  <rect x="24" y="140" width="732" height="72" rx="9" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-poor,#a8320f)" stroke-width="1.6"/>
  <text x="42" y="164" font-size="11.5" font-weight="700" fill="currentColor">Two MATCH clauses — index offline</text>
  <text x="42" y="182" font-size="10" fill="var(--viz-ink-mute,#565f6d)">box predicate correct; index missing or POPULATING</text>
  <rect x="300" y="152" width="150" height="22" rx="11" fill="var(--viz-poor,#a8320f)"/>
  <text x="375" y="168" text-anchor="middle" font-size="10" font-weight="700" fill="var(--viz-on-pill,#ffffff)">Apply + LabelScan</text>
  <text x="42" y="202" font-size="10" fill="var(--viz-poor,#a8320f)" font-weight="600">800,000,000 node reads — plus per-row setup</text>
  <rect x="470" y="152" width="270" height="46" rx="6" fill="var(--viz-panel-2,#ece9df)" stroke="var(--viz-stroke-soft,#cdc6b3)" stroke-width="1"/>
  <rect x="478" y="168" width="254" height="14" rx="7" fill="var(--viz-poor,#a8320f)"/>
  <rect x="24" y="222" width="732" height="72" rx="9" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-good,#0a656d)" stroke-width="2"/>
  <text x="42" y="246" font-size="11.5" font-weight="700" fill="currentColor">Two MATCH clauses — index ONLINE</text>
  <text x="42" y="264" font-size="10" fill="var(--viz-ink-mute,#565f6d)">identical query text to the row above</text>
  <rect x="300" y="234" width="150" height="22" rx="11" fill="var(--viz-good,#0a656d)"/>
  <text x="375" y="250" text-anchor="middle" font-size="10" font-weight="700" fill="var(--viz-on-pill,#ffffff)">Apply + IndexSeek</text>
  <text x="42" y="284" font-size="10" fill="var(--viz-good,#0a656d)" font-weight="600">~60,000 node reads</text>
  <rect x="470" y="234" width="270" height="46" rx="6" fill="var(--viz-panel-2,#ece9df)" stroke="var(--viz-stroke-soft,#cdc6b3)" stroke-width="1"/>
  <rect x="478" y="250" width="19" height="14" rx="7" fill="var(--viz-good,#0a656d)"/>
</svg>
