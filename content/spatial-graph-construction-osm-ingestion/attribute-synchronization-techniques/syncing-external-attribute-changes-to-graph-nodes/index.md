...
<svg viewBox="0 0 880 412" role="img" aria-labelledby="syncTitle syncDesc" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;font-family:var(--font-sans,sans-serif)">
  ...
  <!-- stage headers -->
  <rect class="viz-backdrop" x="0" y="0" width="880" height="412" fill="var(--viz-bg,#ffffff)"/>
...
<svg viewBox="0 0 780 300" role="img" aria-labelledby="lwTitle lwDesc" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;font-family:var(--font-sans,sans-serif)">
  <title id="lwTitle">Two payloads, two commit orders, and only one of them is safe</title>
  <desc id="lwDesc">Version 9 and version 6 for the same node, racing. With a bare SET the surviving value depends on which transaction commits second: if version 6 lands last it overwrites the newer weight and the loader reports success either way, so the corruption is invisible. With the version guard in the query, version 6 matches the node, fails the strictly-less-than comparison and writes nothing, so version 9 survives regardless of commit order. The guard has to be in the statement — application-side ordering cannot see the other transaction.</desc>
  <defs>
    <marker id="lw-a" viewBox="0 0 10 10" refX="8.5" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
      <path d="M0 0 L10 5 L0 10 z" fill="currentColor"/>
    </marker>
  </defs>
  <rect class="viz-backdrop" x="0" y="0" width="780" height="300" fill="var(--viz-bg,#ffffff)"/>
  <text x="24" y="24" font-size="13.5" font-weight="700" fill="currentColor">Same node, same two payloads, commit order reversed</text>
  <rect x="24" y="42" width="356" height="200" rx="10" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-poor,#a8320f)" stroke-width="1.8"/>
  <text x="202" y="66" text-anchor="middle" font-size="11.5" font-weight="700" fill="var(--viz-poor,#a8320f)">bare SET</text>
  <g font-size="10.5" font-weight="700" text-anchor="middle">
    <rect x="48" y="84" width="130" height="28" rx="7" fill="var(--accent-3,#5b21b6)"/><text x="113" y="103" fill="var(--viz-on-pill,#ffffff)">v9 commits</text>
    <rect x="226" y="84" width="130" height="28" rx="7" fill="var(--viz-poor,#a8320f)"/><text x="291" y="103" fill="var(--viz-on-pill,#ffffff)">v6 commits</text>
  </g>
  <line x1="178" y1="98" x2="222" y2="98" stroke="currentColor" stroke-width="1.5" marker-end="url(#lw-a)"/>
  <line x1="202" y1="112" x2="202" y2="140" stroke="currentColor" stroke-width="1.5" marker-end="url(#lw-a)"/>
  <rect x="72" y="142" width="260" height="34" rx="7" fill="var(--viz-panel-2,#ece9df)" stroke="var(--viz-poor,#a8320f)" stroke-width="1.6"/>
  <text x="202" y="164" text-anchor="middle" font-size="11" font-weight="700" fill="var(--viz-poor,#a8320f)">stored weight is v6's</text>
  <text x="202" y="198" text-anchor="middle" font-size="10.5" fill="var(--viz-ink-mute,#565f6d)">both writes reported success</text>
  <text x="202" y="214" text-anchor="middle" font-size="10.5" fill="var(--viz-ink-mute,#565f6d)">nothing distinguishes this run from a correct one</text>
  <rect x="400" y="42" width="356" height="200" rx="10" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-good,#0a656d)" stroke-width="1.8"/>
  <text x="578" y="66" text-anchor="middle" font-size="11.5" font-weight="700" fill="var(--viz-good,#0a656d)">WHERE n.attr_version &lt; u.version</text>
  <g font-size="10.5" font-weight="700" text-anchor="middle">
    <rect x="424" y="84" width="130" height="28" rx="7" fill="var(--accent-3,#5b21b6)"/><text x="489" y="103" fill="var(--viz-on-pill,#ffffff)">v9 commits</text>
    <rect x="602" y="84" width="130" height="28" rx="7" fill="var(--viz-good,#0a656d)"/><text x="667" y="103" fill="var(--viz-on-pill,#ffffff)">v6 no-ops</text>
  </g>
  <line x1="554" y1="98" x2="598" y2="98" stroke="currentColor" stroke-width="1.5" marker-end="url(#lw-a)"/>
  <line x1="578" y1="112" x2="578" y2="140" stroke="currentColor" stroke-width="1.5" marker-end="url(#lw-a)"/>
  <rect x="448" y="142" width="260" height="34" rx="7" fill="var(--viz-panel-2,#ece9df)" stroke="var(--viz-good,#0a656d)" stroke-width="1.6"/>
  <text x="578" y="164" text-anchor="middle" font-size="11" font-weight="700" fill="var(--viz-good,#0a656d)">stored weight is v9's</text>
  <text x="578" y="198" text-anchor="middle" font-size="10.5" fill="var(--viz-ink-mute,#565f6d)">the guard is evaluated against committed state</text>
  <text x="578" y="214" text-anchor="middle" font-size="10.5" fill="var(--viz-ink-mute,#565f6d)">so commit order stops mattering</text>
  <text x="24" y="272" font-size="10.5" fill="var(--viz-ink-mute,#565f6d)">Sorting the batch by version in Python fixes the order inside one loader process and nothing else — a second</text>
  <text x="24" y="288" font-size="10.5" fill="var(--viz-ink-mute,#565f6d)">replica, a retry, or a replayed offset reintroduces the race the moment the guard is not in the statement.</text>
</svg>
