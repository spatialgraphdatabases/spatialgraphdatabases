...
<svg viewBox="8 -2 936 322" role="img" aria-labelledby="pbfTitle pbfDesc" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;font-family:var(--font-sans,sans-serif)">
  ...
  <!-- column captions -->
  <rect class="viz-backdrop" x="8" y="-2" width="936" height="322" fill="var(--viz-bg,#ffffff)"/>
  ...
  <g font-size="12.5" fill="currentColor">
    ...
    <text x="874" y="78" text-anchor="middle" fill="var(--viz-on-pill,#ffffff)" font-weight="600">writer</text>
    <text x="874" y="98" text-anchor="middle" fill="var(--viz-on-pill,#ffffff)" font-size="10.5">execute_write</text>
...
<svg viewBox="0 0 780 288" role="img" aria-labelledby="plTitle plDesc" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;font-family:var(--font-sans,sans-serif)">
  <title id="plTitle">Two places the node locations can live, and only one of them scales</title>
  <desc id="plDesc">Resident memory for a country-scale extract of 220 million nodes. Letting PyOsmium own the location store keeps the mapping in a packed C++ array indexed by node id, roughly eight bytes per node, and the Python handler stays stateless apart from the current batch. Rebuilding the same mapping as a Python dictionary of integer keys to float tuples costs on the order of 150 bytes per entry once object headers, the tuple, and the two boxed floats are counted — the same information, roughly twenty times the memory, and the run is killed before the first commit.</desc>
  <rect class="viz-backdrop" x="0" y="0" width="780" height="288" fill="var(--viz-bg,#ffffff)"/>
  <text x="24" y="24" font-size="13.5" font-weight="700" fill="currentColor">Where the node-id to coordinate mapping lives</text>
  <text x="24" y="42" font-size="10.5" fill="var(--viz-ink-mute,#565f6d)">220 million nodes · one country extract</text>
  <rect x="24" y="58" width="732" height="88" rx="10" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-good,#0a656d)" stroke-width="1.8"/>
  <text x="44" y="82" font-size="11.5" font-weight="700" fill="var(--viz-good,#0a656d)">apply_file(..., locations=True)</text>
  <text x="44" y="100" font-size="10" fill="var(--viz-ink-mute,#565f6d)">packed C++ store, ~8 bytes per node; the handler keeps only the current batch</text>
  <rect x="44" y="112" width="600" height="18" rx="9" fill="var(--viz-panel-2,#ece9df)"/>
  <rect x="44" y="112" width="30" height="18" rx="9" fill="var(--viz-good,#0a656d)"/>
  <text x="660" y="126" font-size="12" font-weight="700" fill="currentColor">~1.8 GB</text>
  <rect x="24" y="158" width="732" height="88" rx="10" fill="var(--viz-panel,#f4f4f5)" stroke="var(--viz-poor,#a8320f)" stroke-width="1.8"/>
  <text x="44" y="182" font-size="11.5" font-weight="700" fill="var(--viz-poor,#a8320f)">a Python dict built in the node callback</text>
  <text x="44" y="200" font-size="10" font-family="var(--font-mono,monospace)" fill="var(--viz-ink-mute,#565f6d)">locs[n.id] = (n.location.lat, n.location.lon)   # ~150 bytes per entry</text>
  <rect x="44" y="212" width="600" height="18" rx="9" fill="var(--viz-panel-2,#ece9df)"/>
  <rect x="44" y="212" width="600" height="18" rx="9" fill="var(--viz-poor,#a8320f)"/>
  <text x="660" y="226" font-size="12" font-weight="700" fill="currentColor">~33 GB</text>
  <text x="24" y="272" font-size="10.5" fill="var(--viz-ink-mute,#565f6d)">The dict is the same index the library already maintains, rebuilt in the one runtime that can least afford it.</text>
</svg>
